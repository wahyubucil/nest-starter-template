...
- Create seeder on `src/database/seeder`. See example on `src/database/seeder/item.seeder.ts`
- Add it to `src/database/seeder/seed.ts` on `runSeeeder()` function
- Run seeder
...
$ rm -rf src/database/seeder/item.seeder.ts
```

Don't forget to remove imported module on `app.module.ts` and `seed.ts`
